...
public class TeamController {
	...
	public ResponseEntity<Optional<Team>> getTeamsById(@PathVariable("id") Long id) {
		...
		if (!team.isPresent()) {
			...
			throw new SomeCustomException(LOGEER_PREFIX + " No Team Found with id=" + id);
		}
	...
	public ResponseEntity<Optional<Team>> updateTeam(@RequestBody @Validated TeamDTO teamWithoutId, @PathVariable("id") Long id) {
		...
		Team team = new Team(id, teamWithoutId.getTeamName(), teamWithoutId.getGameName(), teamWithoutId.getEmailId());
		if (teamService.updateTeam(team)) {
			...
			throw new SomeCustomException(LOGEER_PREFIX + " Could Not Update team with id=" + id);
		}
	}
	...
	public ResponseEntity<Optional<Team>> deleteTeamById(@PathVariable("id") Long id) {
		...
		if (!team.isPresent()) {
			...
			throw new SomeCustomException(LOGEER_PREFIX + " Could not delete team with id=" + id);
		}
		return ResponseEntity.ok(team);
	}
